...
static bool srt_parse_url(char* url, srt_params_t* params)
{
    char* query = NULL;
...
static int srt_set_socket_option(vlc_object_t *this, const char *srt_param,
...
static bool srt_schedule_reconnect(stream_t *p_stream)
{
    ...
    if (psz_passphrase != NULL && psz_passphrase[0] != '\0') {
        ...
        msg_Dbg( p_stream, "SRT: set passphrase='%s'", psz_passphrase );
    }
    ...
    if (psz_streamid != NULL && psz_streamid[0] != '\0') {
        ...
        msg_Dbg( p_stream, "SRT: set streamid='%s'", psz_streamid );
    }
	...
	if (i_payload_size > 0 ) {
        ...
        msg_Dbg( p_stream, "SRT: set payload_size=%d", i_payload_size );
   }
...
vlc_module_begin ()
    ...
    add_string( "streamid", "", "streamid", NULL, false )
